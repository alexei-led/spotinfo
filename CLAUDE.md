...
> and is driven by `.mockery.yaml`. There is no `make mocks` target and no `go:generate`
> directive, so regeneration is a manual `mockery` run.
> **Trap:** `.mockery.yaml` covers `advisorProvider`, `pricingProvider`, `scoreProvider`,
> `awsAPIProvider` and `spotClient` — but NOT `livePriceProvider`, whose mock is hand-written
> inside the generated file. Regenerating silently deletes it and breaks `liveprice_test.go`.
> Add `livePriceProvider` to `.mockery.yaml` before ever running mockery.
