...
// hundreds of kilobytes; the ceiling keeps a corrupted or hostile archive from
...
// LoadEmbeddedSnapshot reads and fully verifies the committed Azure snapshot.
//
...
// costs one gunzip, which is worth paying so a provider is disabled rather than
// serving a snapshot nobody approved.
...
func loadSnapshot(contractJSON, manifestJSON, payload []byte) (*Snapshot, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("azure source contract: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("azure snapshot manifest: %w", err)
	}
